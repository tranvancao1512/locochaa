			...
			if(file_exists($this->_args['option_file'])){
				try {
					...
					foreach($sxe->tab as $tab){
						...
						$tab_titles .= '<li ' . ($tabidx == 1 ? 'class="uk-active"':'') .'><a href="#option-tab-'.$tabidx.'">'.$icon. __(''.$tab_name,'cactusthemes').'</a></li>';
						...
						foreach($tab->group as $group){
							...
							$tab_contents .= 
								'<table class="uk-table" width="100%" cellpadding="0" cellspacing="0">';
									...
									$tab_contents .= '
									<thead>
										<th>'.$icon. __(''.$group_name,'cactusthemes').'</th>
									</thead>';
									}
							...
							foreach($group->fields as $fields){
								...
								$tab_contents .=
									'<tr class="row label '.(count($fields->description) > 0?'no-border':'').'">
										<td align="left">
											<label>'
												. __(''.$fields_atts['label'],'cactusthemes').'</label>
										</td>
									</tr>
									<tr class="row">
										<td>';
											foreach($fields->option as $option){
												...
												if(isset($atts['tooltip'])){
													$props = ' data-uk-tooltip title="' . __(''.$atts['tooltip'],'cactusthemes') . '" ';
												}
												...
												if(isset($atts['label'])){
													$tab_contents .= '<label>' . __(''.$atts['label'],'cactusthemes') . ':</label>';
												}
									...
									if(count($fields->description) > 0){
										foreach($fields->description as $desc){
											$tab_contents .= '<tr class="row description"><td>'.__(''.$desc->asXML(),'cactusthemes').'</td></tr>';
										}
									}
							}
					...
					echo __("Exception occurs: ",'cactusthemes') . print_r($e);
				}
			}
